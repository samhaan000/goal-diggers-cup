...
function clearYoutubeLink(){if(!confirmAction("Clear YouTube link?","This clears the link field only. Tap Save Live Link afterward if you want to save it blank.","clear link"))return;document.getElementById("youtubeLink").value=""}
async function pasteYoutubeLink(){const input=document.getElementById("youtubeLink");try{const text=await navigator.clipboard.readText();if(!text.trim()){alert("Clipboard is empty");return}input.value=text.trim();input.focus()}catch(err){alert("Paste permission was blocked. Tap the field and paste manually.")}}
